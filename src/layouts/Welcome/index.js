...
class Welcome extends Component {
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <Image
            source={item.source}
            resizeMode="contain"
            style={{width: width, height:height/1.8, overflow: 'visible'}}
          />
        )}
  ...
  render() {
    ...
    return (
      <ScrollView>
        <Block style={{backgroundColor: '#fff',}}  height={height-30}>
          {/* <Block center margin={[30, 0, 0, 0]} flex={false}>
            <Text bold size={35} logoColor center>
              Quiz{' '}
              <Text bold size={45} logoMainColor center>
                App
              </Text>
            </Text>
          </Block> */}
          <Block center middle margin={[30, 0, 50, 0]} >
            {this.renderIllustrations()}
            {/* {this.renderSteps()} */}
          </Block>
          <Block middle flex={0.8} margin={[0, theme.sizes.padding]}>
            <Button shadow logoMainColor padding={12} onPress={() => Actions.login()}>
              <Text center semibold white>
                Login
              </Text>
            </Button>
            <Button shadow white padding={12} onPress={() => Actions.signup()}>
              <Text center semibold logoMainColor>
                Sign Up
              </Text>
            </Button>
            <Button transparent shadow padding={12} onPress={() => this.setState({showTerms: true})}>
